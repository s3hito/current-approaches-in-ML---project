...
import torch, math, glob, numpy as np
...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads=1):
        ...
        self.heads = nn.ModuleList(
            [AttentionBlock(input_dim=embed_dim, output_dim=self.embed_per_head) for _ in range(num_heads)])
    ...
    def forward(self, query, key, value):
        ...
        attentions, attention_weights = torch.cat(attentions_list, dim=2), torch.stack(attention_weights_list).mean(
            dim=0)
        return self.projection(attentions), attention_weights


class SoundSequenceAnalyzer(nn.Module):
    def __init__(self, time_window_size=128, cnn_out=64, freq_vec_len=100,
                 num_heads=1):  # embed dim - vector length for a single frequency (n_bins)
        super().__init__()
        self.cnn_preprocessor = CNN(out_size=cnn_out)  # extract local acoustic features

        self.input_projection = nn.Linear(in_features=cnn_out + 9, out_features=freq_vec_len,
                                          bias=False)  # 6 additional features for difficulty parameters
    ...
    def forward(self, spectrogram, difficulty):  # (B, 1, n_mels, T)
        ...
        x = self.input_projection(torch.cat((x, difficulty),
                                            dim=-1))  # (B, Time, embed_dim). Concatenate with difficulty to distingush between different difficulties of the map
...
class CoordMDNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = torch.sigmoid(out[..., self.n_components:self.n_components * 3])  # coords for each of 5 centers
