import { Vector2 } from "./Objects/Vector2";
export declare abstract class Precision {
    static FLOAT_EPSILON: number;
    static almostEqualsNumber(value1: number, value2: number, acceptableDifference?: number): boolean;
    static almostEqualsVector(vec1: Vector2, vec2: Vector2, acceptableDifference?: number): boolean;
    static DefinitelyBigger(value1: number, value2: number, acceptableDifference?: number): boolean;
}
